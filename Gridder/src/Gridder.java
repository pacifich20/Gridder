    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        for (int col = 0; col < gridCount; ++col) {
            for (int row = 0; row < gridCount; ++row) {
                ...
                for (int i = Math.max(0, col - 1); i <= Math.min(gridCount - 1, col + 1); ++i) {
                    for (int j = Math.max(0, row - 1); j <= Math.min(gridCount - 1, row + 1); ++j) {
                        if (i != col || j != row) {
                            numWhite += grid[i][j];
                        }
                    }
                ...
                tempGrid[col][row] = (numWhite == 2) ? 1 : 0;
            }
        }
